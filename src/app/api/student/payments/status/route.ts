...
import { getInvoices, saveInvoices, getSepayTransactions, saveSepayTransactions, fulfillPaidInvoice } from "@/lib/paymentDb";
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (matchedTx) {
            ...
            // 1. Cập nhật hóa đơn và đồng bộ tức thì vai trò + gói cước + hóa đơn Supabase Auth & DB
            const updatedInv = await fulfillPaidInvoice(invoice, "SEPAY", String(matchedTx.id));
            invoice.status = "PAID";
            invoice.paidAt = updatedInv.paidAt;
